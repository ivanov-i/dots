...
**Important:** Use system fonts (variable width) for ALL text elements including code blocks - code should look good in the system font. Only use monospace fonts for aligning numbers in tables when alignment is critical. Never use monospace fonts like 'Courier New' for code unless specifically requested.
...
## Code Display
- No syntax highliting except for comments, which must be bold
- Line numbers for longer code blocks
- Word wrapping for wide code with a nice indication when the line is wrapped
